...
function parseResponseObject (
  ...
  if (FIELD_NAME.CONTENT in responseObject === false) {
...
function parseResponsesObject (
  ...
  for (const [statusCode, _responseObject] of Object.entries(responsesObject)) {
    const responseObject =
      FIELD_NAME.REF in _responseObject
        ? parseReferenceObject(
          inputPath,
          openapiObject,
          _responseObject[FIELD_NAME.REF]
        )
        : _responseObject

    if (
      FIELD_NAME.CONTENT in responseObject &&
      MEDIA_TYPE.APPLICATION_JSON in responseObject[FIELD_NAME.CONTENT] === false
    ) {
      continue
    }
    ...
    if (
      ...
      FIELD_NAME.SCHEMA in responseObject[FIELD_NAME.CONTENT][MEDIA_TYPE.APPLICATION_JSON] === false
    ) {
      continue
    }
