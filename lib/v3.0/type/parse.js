...
function toTypeNode (
  ...
  if (obj[FIELD_NAME.TYPE] === PRIMITIVE_TYPE.BOOLEAN) {
    return ts.factory.createKeywordTypeNode(ts.SyntaxKind.BooleanKeyword)
  }

  if (obj[FIELD_NAME.TYPE] === PRIMITIVE_TYPE.NUMBER) {
...
function parseOperationObject (
  ...
  if (FIELD_NAME.REQUEST_BODY in operationObject) {
    if (FIELD_NAME.CONTENT in operationObject[FIELD_NAME.REQUEST_BODY] === false) {
      ...
      throw new Error(`Request Body Object requires content field. Object: ${JSON.stringify(operationObject[FIELD_NAME.REQUEST_BODY])}`)
    }
