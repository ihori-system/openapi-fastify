...
test('generate from json', async (t) => {
  const equal = async (t, target) =>
    await t.test(target, () => assert.equal(
      printer.printFile(readAndGenerateSchema(path.join('test/v3.1/fixtures/json', `${target}.json`))),
  ...
  const throws = async (t, target) =>
    await t.test(target, () => assert.throws(
      () => readAndGenerateSchema(path.join('test/v3.1/fixtures/json', `${target}.json`))
    ))
...
test('generate from yaml', async (t) => {
  const equal = async (t, target) =>
    await t.test(target, () => assert.equal(
      printer.printFile(readAndGenerateSchema(path.join('test/v3.1/fixtures/yaml', `${target}.yaml`))),
      fs.readFileSync(path.join(__dirname, '../fixtures/schema', `${target}.ts`), 'utf8')
    ))
...
test('unknown file type', async (t) => {
  assert.throws(() => readAndGenerateSchema('test/v3.1/fixtures/bar/foo.bar'))
})

test('unknown version', async (t) => {
  assert.throws(() => readAndGenerateSchema('test/unknown/fixtures/unknown-version.json'))
})
